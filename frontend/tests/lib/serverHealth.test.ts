...
describe("resolveServerHealthUrl", () => {
  ...
  it("keeps same-origin production URLs relative", () => {
    expect(resolveServerHealthUrl("/api/v1", "https://app.aurum-pharma.tj")).toBe(
      "/healthz",
    );
  });
  ...
  it("drops path, query, and hash from configured API URLs", () => {
    expect(
      resolveServerHealthUrl("https://api.aurum-pharma.tj/proxy/api/v1?x=1#top"),
    ).toBe("https://api.aurum-pharma.tj/healthz");
  });
});
