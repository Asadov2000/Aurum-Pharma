...
- [x] `refund_reason_mode` требует допустимый управляемый код причины и, в режиме
      `required_with_text`, служебный комментарий. Комментарий не попадает в
      платёжный metadata или audit; новые строки защищены DB constraint.
