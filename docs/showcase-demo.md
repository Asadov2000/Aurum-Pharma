...
В постоянном локальном demo включён `AUTH_LOCAL_DEMO_OWNER_LOGIN`: существующий
тестовый владелец входит по коду из почты (показан в dev-режиме) и своему паролю,
без кода из приложения-аутентификатора. Обновление сессии сохраняет этот режим.
Подключённый второй фактор и резервные коды не удаляются, а сессия не помечается
как прошедшая MFA. Для критических действий требуется обычный вход с MFA:
выключите эту опцию в demo-конфигурации и войдите заново.
Для остальных аккаунтов политика входа не меняется. Опция по умолчанию выключена;
она разрешена только в development с `AUTH_LOCAL_TESTING_MODE`, БД `aurum_demo`
для обоих DB-пулов и явными локальными CORS origins. Staging/production с этой
опцией не запускаются. Это исключение для демонстрации, не настройка рабочего пилота.
