...
async def list_incoming(
    _user: Annotated[CurrentUser, Depends(require_permission("incoming.view"))],
...
async def get_incoming(
    ...
    items = await service.list_items(document_id)
...
async def add_item(
    ...
    _user: Annotated[CurrentUser, Depends(require_permission("incoming.create"))],
    service: Annotated[IncomingService, Depends(_service)],
) -> IncomingItemRead:
    item = await service.add_item(document_id, fields=payload.model_dump())
...
async def update_item(
    ...
    item = await service.update_item(
        document_id, item_id, fields=payload.model_dump(exclude_none=True)
    )
    return IncomingItemRead.model_validate(item)
...
async def delete_item(
    ...
    await service.delete_item(document_id, item_id)
...
async def accept_incoming(
    ...
    doc = await service.accept(document_id, actor_id=user.user_id)
    return IncomingDocumentRead.model_validate(doc)
...
async def reject_incoming(
    ...
    doc = await service.reject(document_id, actor_id=user.user_id)
    return IncomingDocumentRead.model_validate(doc)
